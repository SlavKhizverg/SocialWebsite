...
// 		exit();
	...
	$sql = "INSERT INTO posts(name, author, email, data,type) 
			VALUES('$account_name','$log_username','$email','$data')";
	$query = mysqli_query($conn, $sql);
	$id = mysqli_insert_id($conn);
	mysqli_query($conn, "UPDATE status SET id='$id' WHERE id='$id' LIMIT 1");
	// Count posts of type "a" for the person posting and evaluate the count
	$sql = "SELECT COUNT(id) FROM post WHERE author='$log_username' AND type='a'";
	$query = mysqli_query($conn, $sql); 
	$row = mysqli_fetch_row($query);
	if ($row[0] > 9) { // If they have 10 or more posts of type a
		...
		$row = mysqli_fetch_row($query);
		$oldest = $row[0];
		mysqli_query($conn, "DELETE FROM post WHERE id='$oldest'");
	}
...
if (isset($_POST['action']) && $_POST['action'] == "status_reply"){
	...
	$id = preg_replace('#[^0-9]#', '', $_POST['sid']);
	$account_name = preg_replace('#[^a-z0-9]#i', '', $_POST['user']);
	$data = htmlentities($_POST['data']);
	$data = mysqli_real_escape_string($conn, $data);
	// Make sure account name exists (the profile being posted on)
	$sql = "SELECT COUNT(id) FROM reg WHERE name='$account_name'";
	$query = mysqli_query($conn, $sql);
	$row = mysqli_fetch_row($query);
	if($row[0] < 1){
		mysqli_close($conn);
	...
	$sql = "INSERT INTO status(id, account_name, author, type, data, postdate)
