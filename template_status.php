...
$statuslist = "";
if($isOwner == "yes"){
	$status_ui = '<textarea id="statustext" onkeyup="statusMax(this,250)" placeholder="What&#39;s new with you '.$name.'?"></textarea>';
	$status_ui .= '<button id="statusBtn" onclick="postToStatus(\'status_post\',\'a\',\''.$name.'\',\'statustext\')">Post</button>';
} else {
	$status_ui = '<textarea id="statustext" onkeyup="statusMax(this,250)" placeholder="Hi visitorName, say something to '.$name.'"></textarea>';
	$status_ui .= '<button id="statusBtn" onclick="postToStatus(\'status_post\',\'c\',\''.$name.'\',\'statustext\')">Post</button>';
}
?><?php 
$sql = "SELECT * FROM posts WHERE name='$name' AND type='a' OR name='$name' AND type='c' ";
...
// 	$data = stripslashes($data);
 	$statusDeleteButton = '';
	if($author == $name || $account_name == $name ){
...
// 			$replydata = stripslashes($replydata);
			$replyDeleteButton = '';
			if($replyauthor == $name || $account_name == $name ){
	...
	if($isFriend == true ){
	    $statuslist .= '<textarea id="replytext_'.$statusid.'" class="replytext" onkeyup="statusMax(this,250)" placeholder="write a comment here"></textarea><button id="replyBtn_'.$statusid.'" onclick="replyToStatus('.$statusid.',\''.$name.'\',\'replytext_'.$statusid.'\',this)">Reply</button>';	
	}
}
?>
<script>
	...
	ajax.onreadystatechange = function() {
		if(ajaxReturn(ajax) == true) {
			...
			if(datArray[0] == "post_ok"){
				...
				_("statusarea").innerHTML = '<div id="status_'+sid+'" class="status_boxes"><div><b>Posted by you just now:</b> <span id="sdb_'+sid+'"><a href="#" onclick="return false;" onmousedown="deleteStatus(\''+sid+'\',\'status_'+sid+'\');" title="DELETE THIS STATUS AND ITS REPLIES">delete status</a></span><br />'+data+'</div></div><textarea id="replytext_'+sid+'" class="replytext" onkeyup="statusMax(this,250)" placeholder="write a comment here"></textarea><button id="replyBtn_'+sid+'" onclick="replyToStatus('+sid+',\'<?php echo $name; ?>\',\'replytext_'+sid+'\',this)">Reply</button>'+currentHTML;
